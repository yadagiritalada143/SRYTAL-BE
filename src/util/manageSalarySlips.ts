...
const generateSalarySlipFileName = (
  employeeName: string,
  payPeriod: string
): string => {
  const months = [
...
const uploadSalarySlipToS3 = async (
  params: IUploadSalarySlipParams
  ...
  console.warn(
    '[ManageSalarySlips] PDF Buffer size:',
    pdfBuffer ? pdfBuffer.length + ' bytes' : 'NO BUFFER'
  );

  if (!pdfBuffer || pdfBuffer.length === 0) {
    console.error(
      '[ManageSalarySlips] ERROR: PDF buffer is empty or undefined!'
    );
    ...
    return new Promise((resolve, reject) => {
      s3Client.upload(uploadParams, (error: any, data: any) => {
        if (error) {
          ...
          console.error(
            '[ManageSalarySlips] Full Error Object:',
