...
import {
  COURSE_MODULE_SUCCESS_MESSAGES,
  COURSE_MODULE_ERRORS_MESSAGES,
} from '../../constants/contentwriter/coursemoduleMessages';
...
const addModuleToCourse = async (req: Request, res: Response) => {
  try {
    ...
    if (req.file) {
      ...
      uniqueThumbnailKey = await uploadThumbnailToS3
        .uploadThumbnailToS3(
          uniqueName,
          buffer,
          mimetype,
    ...
    const responseAfteraddingCourseModule =
      await addCourseModuleService.addNewCourseModule(
        courseId,
        moduleName,
        moduleDescription,
        uniqueThumbnailKey,
