    ...
    .then((responseAfteraddingEmploymentType) => {
      if (responseAfteraddingEmploymentType.id) {
        return res
          ...
          .json({
            message:
              EMPLOYMENT_TYPE_SUCCESS_MESSAGES.EMPLOYMENT_TYPE_ADD_SUCCESS_MESSAGE,
          });
      } else {
        return res
          .status(400)
          .json({
            message:
              EMPLOYMENT_TYPE_ERRORS_MESSAGES.EMPLOYMENT_TYPE_ADD_ERROR_MESSAGE,
          });
      }
    })
    .catch((error) => {
      ...
      return res
        ...
        .json({
          message:
            EMPLOYMENT_TYPE_ERRORS_MESSAGES.EMPLOYMENT_TYPE_UNEXPECTED_ERROR_MESSAGE,
        });
    });
};
