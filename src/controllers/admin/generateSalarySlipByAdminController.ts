...
const generateSalarySlip = async (req: Request, res: Response) => {
  ...
  try {
    ...
    console.warn(
      '[SalarySlipController] Request Body:',
      JSON.stringify({
    ...
    console.warn(
      '[SalarySlipController] Calling generateSalarySlipPDF service...'
    );
    const result =
      await generateSalarySlipByAdminService.generateSalarySlipPDF(
        salarySlipRequest
      );
    console.warn(
      '[SalarySlipController] PDF Generation Result:',
      JSON.stringify({
    ...
    if (result.success && result.pdfBuffer) {
      console.warn(
        '[SalarySlipController] PDF generated successfully, sending response...'
      );
      ...
      console.warn(
        '[SalarySlipController] Initiating email notification (async)...'
      );
      sendSalarySlipNotificationEmail
        ...
        .then(() => {
          console.warn(
            '[SalarySlipController] Email notification completed successfully'
          );
        })
        .catch((error) => {
          ...
          console.error(
            '[SalarySlipController] Email Error Stack:',
            error.stack
          );
        });
      ...
      manageSalarySlips
        ...
        .then((uploadResult) => {
          console.warn(
            '[SalarySlipController] S3 upload completed:',
        ...
        .catch((error) => {
          ...
          console.error(
            '[SalarySlipController] S3 Error:',
            error.message || error.error
          );
          console.error(
            '[SalarySlipController] S3 Error Details:',
            JSON.stringify(error)
          );
        });

      console.warn(
        '[SalarySlipController] Response sent, async tasks initiated'
      );
      return;
    } else {
      ...
      return res.status(HTTP_STATUS.BAD_REQUEST).json({
        ...
        message:
          result.error ||
          SALARY_SLIP_ERROR_MESSAGES.SALARY_SLIP_GENERATION_FAILED,
      });
    }
  } catch (error: any) {
    console.error(
      '[SalarySlipController] EXCEPTION in generateSalarySlip controller!'
    );
