...
const downloadSalarySlip = async (
  ...
  try {
    ...
    if (mongoId !== authenticatedUserId) {
      const currentUser =
        await UserModel.findById(authenticatedUserId).select('userRole');

      const isAdmin =
        currentUser && ADMIN_ROLES.includes(currentUser.userRole || '');
    ...
    if (!result.success) {
      ...
      return res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json({
        success: false,
        message:
          EMPLOYEE_SALARY_SLIP_ERROR_MESSAGES.EMPLOYEE_SALARY_SLIP_DOWNLOADED_ERROR,
      });
    }
    ...
    res.status(HTTP_STATUS.OK).json({
      success: true,
      message:
        EMPLOYEE_SALARY_SLIP_SUCCESS_MESSAGES.EMPLOYEE_SALARY_SLIP_DOWNLOADED,
      data: {
