...
import {
  IPoolCompaniesResponse,
  IPoolCompanyResponse,
} from '../../interfaces/poolcompanies';
...
const getPoolCompanyDetails = (req: Request, res: Response) => {
  manageRecruiterService
    ...
    .catch((error: any) => {
      console.error(
        `Error while fetching pool company details at controller level: ${error}`
      );
      res.status(500).json({
        success: false,
...
const addPoolCompany = (req: Request, res: Response) => {
  manageRecruiterService
    ...
    .catch((error: any) => {
      console.error(
        `Error while adding pool company details at controller level: ${error}`
      );
      res.status(500).json({
        success: false,
