...
} from '../../util/pdfGenerator/numberToWords';
...
const calculateSalaryComponents = (
  request: ISalarySlipRequest
): ISalaryCalculations => {
  const basicSalary = request.basicSalary;
  const hraPercentage =
    request.hraPercentage ?? SALARY_CALCULATION_DEFAULTS.HRA_PERCENTAGE;
  const hra = Math.round((basicSalary * hraPercentage) / 100);
  const conveyanceAllowance =
    request.conveyanceAllowance ??
    SALARY_CALCULATION_DEFAULTS.CONVEYANCE_ALLOWANCE;
  const medicalAllowance =
    request.medicalAllowance ?? SALARY_CALCULATION_DEFAULTS.MEDICAL_ALLOWANCE;
  ...
  const pfPercentage =
    request.pfPercentage ?? SALARY_CALCULATION_DEFAULTS.PF_PERCENTAGE;
  const providentFund = Math.round((basicSalary * pfPercentage) / 100);
  const professionalTax =
    request.professionalTax ?? SALARY_CALCULATION_DEFAULTS.PROFESSIONAL_TAX;
  ...
  return {
    ...
    specialAllowance: Number(
      formatIndianCurrency(specialAllowance).replace(/,/g, '')
    ),
    conveyanceAllowance: Number(
      formatIndianCurrency(conveyanceAllowance).replace(/,/g, '')
    ),
    medicalAllowance: Number(
      formatIndianCurrency(medicalAllowance).replace(/,/g, '')
    ),
    otherAllowances: Number(
      formatIndianCurrency(otherAllowances).replace(/,/g, '')
    ),
    grossEarnings: Number(
      formatIndianCurrency(grossEarnings).replace(/,/g, '')
    ),
    lossOfPayAmount: Number(
      formatIndianCurrency(lopDeduction).replace(/,/g, '')
    ),
    providentFund: Number(
      formatIndianCurrency(providentFund).replace(/,/g, '')
    ),
    professionalTax: Number(
      formatIndianCurrency(professionalTax).replace(/,/g, '')
    ),
    incomeTax: Number(formatIndianCurrency(incomeTax).replace(/,/g, '')),
    otherDeductions: Number(
      formatIndianCurrency(otherDeductions).replace(/,/g, '')
    ),
    totalDeductions: Number(
      formatIndianCurrency(totalDeductions).replace(/,/g, '')
    ),
...
const generateSalarySlipPDF = async (
  request: ISalarySlipRequest
): Promise<IPDFGenerationResult & { calculations?: ISalaryCalculations }> => {
  ...
  console.warn(
    '[SalarySlipService] Request Employee Name:',
  ...
  try {
    ...
    if (!validation.isValid) {
      ...
      console.error(
        '[SalarySlipService] Validation Errors:',
        validation.errors
      );
      return {
        success: false,
    ...
    console.warn(
      '[SalarySlipService] HTML content generated, length:',
      htmlContent.length
    );
    ...
    console.warn(
      '[SalarySlipService] PDF generation result:',
      JSON.stringify({
