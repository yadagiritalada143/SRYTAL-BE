...
import {
  IDownloadSalarySlipRequest,
  IDownloadSalarySlipResult,
} from '../../interfaces/salarySlip';
import { bucketName, salarySlipsFolder } from '../../config/awsS3Config';

const downloadSalarySlip = async (
  params: IDownloadSalarySlipRequest
): Promise<IDownloadSalarySlipResult> => {
  ...
  try {
    ...
    if (error.code === 'NotFound' || error.code === 'NoSuchKey') {
      console.error(
        `Salary slip not found: ${salarySlipsFolder}/${mongoId}/${fullName}-${month}-${year}.pdf`
      );
      return {
        success: false,
