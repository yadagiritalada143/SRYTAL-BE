...
const getPoolCompanyDetails = (): Promise<any> => {
  return new Promise((resolve, reject) => {
    PoolCompaniesModel.find()
      ...
      .catch((error: any) => {
        console.error(
          `Error in fetching pool companies list at service level: ${error}`
        );
        reject({ success: false });
      });
  });
...
const addPoolCompany = async (
  companyDetailsToAdd: IPoolcompanies
): Promise<any> => {
  companyDetailsToAdd.createdAt = new Date();
  companyDetailsToAdd.lastUpdatedAt = new Date();
  const poolCompanyDataToSave: any = new PoolCompaniesModel({
    ...companyDetailsToAdd,
  });
