...
const updateEmployeeTimesheet = async (updateEmployeeTimesheetPayload) => {
    try {
        ...
        const leaveNotificationKeys = new Set();
...
const detectLeaveNotification = (dbTimesheet, payloadTimesheet) => {
...
const getAdminEmails = async () => {
    const admins = await userModel_1.default.find({ userRole: 'admin', isDeleted: { $ne: true } }).select('email').lean();
    const emails = admins.map((admin) => admin && admin.email).filter((email) => email);
    if (emails.length > 0)
        return emails;
    if (process.env.ADMIN_EMAIL_ABOUT_CUSTOMER)
        return [process.env.ADMIN_EMAIL_ABOUT_CUSTOMER];
    return [];
};
...
const dispatchLeaveEmailNotifications = async (employeeId, notifications) => {
    const employee = await userModel_1.default.findById(employeeId).lean();
    if (!employee) {
    ...
    for (const notification of notifications) {
        ...
        if (notification.type === LEAVE_EMAIL_TYPE.REQUEST) {
            ...
            await sendLeaveRequestEmail_1.default.sendLeaveRequestToAdmin(Object.assign(Object.assign({}, commonDetails), { adminEmails }));
        }
            ...
            await sendLeaveRequestEmail_1.default.sendLeaveApprovalEmail(commonDetails);
        }
            ...
            await sendLeaveRequestEmail_1.default.sendLeaveRejectionEmail(Object.assign(Object.assign({}, commonDetails), { rejectionReason: notification.rejectionReason }));
        }
    }
};
