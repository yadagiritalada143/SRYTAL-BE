...
const getDashboardStatsByAdmin = async (organizationId, userId) => {
    var _a, _b;
    const now = new Date();
    const currentMonth = now.getMonth();
    const monthStart = new Date(now.getFullYear(), now.getMonth(), 1);
    const monthEnd = new Date(now.getFullYear(), now.getMonth() + 1, 1);
    const employees = await userModel_1.default.find({
    ...
    for (const emp of employees) {
        ...
        const dept = ((_a = emp.department) === null || _a === void 0 ? void 0 : _a.departmentName) || 'Unassigned';
        departmentBreakdown[dept] = (departmentBreakdown[dept] || 0) + 1;
        const empType = ((_b = emp.employmentType) === null || _b === void 0 ? void 0 : _b.employmentType) || 'Unspecified';
        employmentTypeBreakdown[empType] = (employmentTypeBreakdown[empType] || 0) + 1;
        if (emp.passwordResetRequired === 'true')
    ...
    const recentHires = employees
        ...
        return ({
            ...
            userRole: e.userRole,
            department: ((_a = e.department) === null || _a === void 0 ? void 0 : _a.departmentName) || null,
            designation: Array.isArray(e.employeeRole) && e.employeeRole.length
                ? e.employeeRole.map((r) => r.designation).filter(Boolean).join(', ')
                : null,
    ...
    for (const empPkg of employeePackages) {
        for (const pkg of empPkg.packages || []) {
            ...
            if (project._id)
                activeProjects.add(String(project._id));
            for (const task of pkg.tasks || []) {
                for (const ts of (task.timesheet || [])) {
                    if (ts.status === TIMESHEET_STATUS.WAITING)
                    ...
                    if (tsDate && !isNaN(tsDate.getTime()) && tsDate >= monthStart && tsDate < monthEnd) {
                        ...
                        if (hours > 0)
                            employeesWithTimesheets.add(String(empPkg.employeeId));
                    }
                }
            }
        }
...
exports.default = { getDashboardStatsByAdmin };
