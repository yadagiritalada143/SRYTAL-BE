...
const generatePDF_1 = __importDefault(require("../../util/pdfGenerator/generatePDF"));
const salarySlipTemplate_1 = __importDefault(require("../../util/pdfGenerator/templates/salarySlipTemplate"));
const numberToWords_1 = require("../../util/pdfGenerator/numberToWords");
console.warn('[SalarySlipService] Module loaded');
...
`;
const getPdfFooterTemplate = (payDate) => `
...
const calculateSalaryComponents = (request) => {
    var _a, _b, _c, _d, _e, _f, _g, _h, _j, _k;
    const basicSalary = request.basicSalary;
    const hraPercentage = (_a = request.hraPercentage) !== null && _a !== void 0 ? _a : SALARY_CALCULATION_DEFAULTS.HRA_PERCENTAGE;
    const hra = Math.round((basicSalary * hraPercentage) / 100);
    const conveyanceAllowance = (_b = request.conveyanceAllowance) !== null && _b !== void 0 ? _b : SALARY_CALCULATION_DEFAULTS.CONVEYANCE_ALLOWANCE;
    ...
    const pfPercentage = (_g = request.pfPercentage) !== null && _g !== void 0 ? _g : SALARY_CALCULATION_DEFAULTS.PF_PERCENTAGE;
    ...
    return {
        ...
        hra: Number((0, numberToWords_1.formatIndianCurrency)(hra).replace(/,/g, '')),
        specialAllowance: Number((0, numberToWords_1.formatIndianCurrency)(specialAllowance).replace(/,/g, '')),
        ...
        otherDeductions: Number((0, numberToWords_1.formatIndianCurrency)(otherDeductions).replace(/,/g, '')),
        totalDeductions: Number((0, numberToWords_1.formatIndianCurrency)(totalDeductions).replace(/,/g, '')),
        netPay: Number((0, numberToWords_1.formatIndianCurrency)(netPay).replace(/,/g, '')),
        netPayInWords,
    };
};
...
const validateRequest = (request) => {
    ...
    if (!request.payPeriod)
        errors.push('Pay Period is required');
    if (!request.payDate)
        errors.push('Pay Date is required');
    if (!request.basicSalary || request.basicSalary <= 0)
        errors.push('Valid Basic Salary is required');
    if (!request.totalWorkingDays || request.totalWorkingDays <= 0)
        errors.push('Total Working Days is required');
    if (!request.daysWorked || request.daysWorked < 0)
        errors.push('Days Worked is required');
    return {
        isValid: errors.length === 0,
        errors,
    };
};
const generateSalarySlipPDF = async (request) => {
    console.warn('[SalarySlipService] generateSalarySlipPDF called');
    console.warn('[SalarySlipService] Request Employee ID:', request.employeeId);
    console.warn('[SalarySlipService] Request Employee Name:', request.employeeName);
    console.warn('[SalarySlipService] Request Pay Period:', request.payPeriod);
    try {
        // Validate request
        console.warn('[SalarySlipService] Validating request...');
        const validation = validateRequest(request);
        if (!validation.isValid) {
            console.error('[SalarySlipService] Validation FAILED!');
            console.error('[SalarySlipService] Validation Errors:', validation.errors);
            return {
                success: false,
                error: `Validation failed: ${validation.errors.join(', ')}`,
            };
        }
        console.warn('[SalarySlipService] Validation passed');
        console.warn('[SalarySlipService] Preparing salary slip data...');
        const salarySlipData = prepareSalarySlipData(request);
        console.warn('[SalarySlipService] Salary slip data prepared');
        console.warn('[SalarySlipService] Injecting data into template...');
        const htmlContent = generatePDF_1.default.injectDataIntoTemplate(salarySlipTemplate_1.default, salarySlipData);
        console.warn('[SalarySlipService] HTML content generated, length:', htmlContent.length);
        console.warn('[SalarySlipService] Generating PDF with header/footer...');
        ...
        if (pdfResult.success) {
            ...
            console.warn('[SalarySlipService] Generated file name:', fileName);
            return Object.assign(Object.assign({}, pdfResult), { fileName, calculations: salarySlipData.calculations });
        }
        console.error('[SalarySlipService] PDF generation was not successful');
        return pdfResult;
    }
    catch (error) {
        console.error('[SalarySlipService] EXCEPTION in generateSalarySlipPDF!');
        console.error('[SalarySlipService] Error Name:', error.name);
        console.error('[SalarySlipService] Error Message:', error.message);
        console.error('[SalarySlipService] Error Stack:', error.stack);
        return {
            success: false,
