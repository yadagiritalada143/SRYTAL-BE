...
describe('addCourseService', () => {
    ...
    it('saves the course and returns the saved document', async () => {
        const savedCourse = { _id: 'c1', courseName: 'React', courseDescription: 'Frontend', thumbnail: 'thumb.png', status: 'ACTIVE' };
        saveSpy.mockResolvedValue(savedCourse);
        const result = await addCourseService_1.default.addCourse('React', 'Frontend', 'thumb.png', 'ACTIVE');
        expect(CourseModelMock).toHaveBeenCalledTimes(1);
        expect(CourseModelMock).toHaveBeenCalledWith({
            courseName: 'React',
