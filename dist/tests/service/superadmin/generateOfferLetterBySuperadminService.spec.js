...
describe('generateOfferLetterBySuperadminService', () => {
    ...
    it('generates an offer letter PDF with the provided details', async () => {
        await generateOfferLetterBySuperadminService_1.default.generateOfferLetterBySuperadmin(res, 'John Doe', 'Offer Letter', 'Software Engineer', '2026-01-01', '1000000', 'Bangalore');
        expect(mockSetHeader).toHaveBeenCalledWith('Content-Type', 'application/pdf');
        expect(mockSetHeader).toHaveBeenCalledWith('Content-Disposition', 'attachment; filename=John Doe_OfferLetter.pdf');
