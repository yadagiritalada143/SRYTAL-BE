...
const sendResetPasswordMailMock = sendResetPasswordMail_1.default.sendResetPasswordMail;
describe('employeePasswordResetByAdminService', () => {
    ...
    it('returns { success: false } when the user does not exist', async () => {
        ...
        const result = await employeePasswordResetByAdminService_1.default.employeePasswordResetByAdmin('emp1');
    ...
    it('resets the password, sends the mail and returns success', async () => {
        ...
        expect(sendResetPasswordMailMock).toHaveBeenCalledWith('John', 'Doe', 'john@example.com', hashPasswordMock.mock.calls[0][0]);
        expect(UserModelMock.findOneAndUpdate).toHaveBeenCalledTimes(1);
        expect(UserModelMock.findOneAndUpdate).toHaveBeenCalledWith({ _id: 'emp1' }, { password: 'hashed-password', passwordResetRequired: 'true' }, { new: true });
    ...
    it('propagates errors thrown during the reset flow', async () => {
        ...
        await expect(employeePasswordResetByAdminService_1.default.employeePasswordResetByAdmin('emp1')).rejects.toThrow(resetError);
        expect(UserModelMock.findOne).toHaveBeenCalledTimes(1);
    });
});
