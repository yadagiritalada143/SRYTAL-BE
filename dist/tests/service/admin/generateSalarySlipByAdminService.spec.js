...
let generateSalarySlipByAdminService;
jest.mock('../../../util/pdfGenerator/generatePDF', () => ({
    __esModule: true,
    default: {
        injectDataIntoTemplate: jest.fn((template, data) => `<html>${template}</html>`),
...
jest.mock('../../../util/pdfGenerator/numberToWords', () => ({
    convertAmountToWords: jest.fn((n) => `${n} rupees`),
    formatIndianCurrency: jest.fn((n) => String(n)),
}));
const pdfGeneratorMock = generatePDF_1.default;
const convertAmountToWordsMock = numberToWords_1.convertAmountToWords;
const formatIndianCurrencyMock = numberToWords_1.formatIndianCurrency;
const buildRequest = (overrides = {}) => (Object.assign({ _id: 'slip1', employeeId: 'EMP-1', employeeName: 'John Doe', employeeEmail: 'john@x.com', designation: 'Developer', department: 'IT', dateOfJoining: '2024-01-01', payPeriod: 'July 2026', payDate: '2026-07-25', bankName: 'HDFC', IFSCCODE: 'HDFC0001', bankAccountNumber: '123456', transactionType: 'NEFT', transactionId: 'TXN1', panNumber: 'PAN123', uanNumber: 'UAN123', totalWorkingDays: 26, daysWorked: 24, lossOfPayDays: 2, basicSalary: 50000, hraPercentage: 10, specialAllowance: 3000, conveyanceAllowance: 2000, medicalAllowance: 1500, otherAllowances: 1000, pfPercentage: 12, professionalTax: 200, incomeTax: 500, otherDeductions: 100 }, overrides));
describe('generateSalarySlipByAdminService', () => {
    ...
    beforeEach(() => {
        ...
        convertAmountToWordsMock.mockImplementation((n) => `${n} rupees`);
        formatIndianCurrencyMock.mockImplementation((n) => String(n));
        pdfGeneratorMock.generatePDFWithHeaderFooter.mockResolvedValue({
            success: true,
            pdfBuffer: Buffer.from('PDF-DATA'),
        });
        pdfGeneratorMock.injectDataIntoTemplate.mockImplementation((template, data) => `<html>${template}</html>`);
    });
    afterEach(() => {
    ...
    describe('validateRequest', () => {
        ...
        it('rejects an invalid pay date', () => {
            const validation = generateSalarySlipByAdminService.validateRequest(buildRequest({ payDate: '2026-02-30' }));
            expect(validation.isValid).toBe(false);
            expect(validation.errors.join(', ')).toContain('Pay date 2026-02-30 is invalid');
        });
        it('rejects a completely unparseable pay date', () => {
            const validation = generateSalarySlipByAdminService.validateRequest(buildRequest({ payDate: 'not-a-date' }));
            expect(validation.isValid).toBe(false);
        });
        it('rejects non-positive basic salary, total working days and negative days worked', () => {
            const validation = generateSalarySlipByAdminService.validateRequest(buildRequest({ basicSalary: 0, totalWorkingDays: 0, daysWorked: -1 }));
            expect(validation.isValid).toBe(false);
            expect(validation.errors).toContain('Valid Basic Salary is required');
            expect(validation.errors).toContain('Total Working Days is required');
            expect(validation.errors).toContain('Days Worked is required');
    ...
    describe('generateSalarySlipPDF', () => {
        it('generates a successful pdf and returns file name plus calculations', async () => {
            ...
            expect(pdfGeneratorMock.generatePDFWithHeaderFooter.mock.calls[0][2]).toContain('Pay Date: 25-Jul-2026');
    ...
    describe('prepareSalarySlipData', () => {
        it('defaults optional display values', () => {
            const data = generateSalarySlipByAdminService.prepareSalarySlipData(buildRequest({ transactionId: undefined, uanNumber: undefined, lossOfPayDays: undefined }));
