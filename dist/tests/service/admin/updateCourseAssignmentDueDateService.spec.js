...
const findByIdMock = courseAssignmentModel_1.default.findById;
const userFindByIdMock = userModel_1.default.findById;
const courseFindByIdMock = coursesModel_1.default.findById;
const sendCourseDueDateUpdateEmailMock = sendCourseDueDateUpdateEmail_1.default.sendCourseDueDateUpdateEmail;
...
describe('updateCourseAssignmentDueDateService', () => {
    ...
    it('rejects when the due date is invalid', async () => {
        await expect(updateCourseAssignmentDueDateService_1.default.updateCourseAssignmentDueDate('a1', null)).rejects.toThrow('Invalid due date');
        expect(findByIdMock).not.toHaveBeenCalled();
    });
    it('rejects with the not found message when the assignment does not exist', async () => {
        findByIdMock.mockResolvedValue(null);
        await expect(updateCourseAssignmentDueDateService_1.default.updateCourseAssignmentDueDate('a1', new Date('2026-12-01'))).rejects.toThrow(courseAssignmentMessages_1.COURSE_ASSIGNMENT_ERRORS_MESSAGES.COURSE_ASSIGNMENT_DETAILS_NOT_FOUND_MESSAGE);
    ...
    it('updates the due date and dispatches the email when extended', async () => {
        ...
        userFindByIdMock.mockReturnValue(buildSelectLeanQuery({ _id: 'e1', firstName: 'John', lastName: 'Doe', email: 'john@x.com' }));
        courseFindByIdMock.mockReturnValue(buildSelectLeanQuery({ _id: 'c1', courseName: 'Intro' }));
        const result = await updateCourseAssignmentDueDateService_1.default.updateCourseAssignmentDueDate('a1', newDueDate);
        expect(findByIdMock).toHaveBeenCalledTimes(1);
        expect(findByIdMock).toHaveBeenCalledWith('a1');
    ...
    it('dispatches the email with isExtended false when the due date is moved earlier', async () => {
        ...
        userFindByIdMock.mockReturnValue(buildSelectLeanQuery({ _id: 'e1', firstName: 'John', lastName: 'Doe', email: 'john@x.com' }));
        courseFindByIdMock.mockReturnValue(buildSelectLeanQuery({ _id: 'c1', courseName: 'Intro' }));
        await updateCourseAssignmentDueDateService_1.default.updateCourseAssignmentDueDate('a1', newDueDate);
        expect(sendCourseDueDateUpdateEmailMock).toHaveBeenCalledTimes(1);
        expect(sendCourseDueDateUpdateEmailMock).toHaveBeenCalledWith(expect.objectContaining({ isExtended: false }));
    });
