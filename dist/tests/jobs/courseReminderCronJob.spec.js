...
const cronScheduleMock = node_cron_1.default.schedule;
const assignmentFindMock = courseAssignmentModel_1.default.find;
const assignmentUpdateMock = courseAssignmentModel_1.default.findByIdAndUpdate;
const userFindMock = userModel_1.default.find;
const sendCourseReminderEmailMock = sendCourseReminderEmail_1.default.sendCourseReminderEmail;
const NOW_IST_1130 = new Date('2026-06-10T06:00:00.000Z');
const BUILD_USER_QUERY = (employees) => ({
    select: jest.fn().mockReturnThis(),
    lean: jest.fn().mockResolvedValue(employees)
});
const buildAssignmentQuery = (result) => ({
    populate: jest.fn().mockReturnThis(),
    sort: jest.fn().mockReturnThis(),
    lean: jest.fn().mockReturnThis(),
    exec: jest.fn().mockResolvedValue(result)
});
const makeAssignment = (overrides) => (Object.assign({ _id: 'a1', employeeId: 'e1', courseId: 'c1', status: 'Assigned', assignedAt: new Date('2026-06-01T05:00:00.000Z'), dueDate: new Date('2026-07-01T00:00:00.000Z'), reminderScheduleStartDate: new Date('2026-06-01T05:00:00.000Z'), lastReminderSentAt: null }, overrides));
...
describe('courseReminderCronJob', () => {
    ...
    it('is scheduled to run at 11:30 AM Indian Standard Time', () => {
        expect(cronScheduleMock).toHaveBeenCalledWith('30 11 * * *', expect.any(Function), { timezone: 'Asia/Kolkata' });
    });
    ...
    it('sends the first reminder 3 days after the course assignment date', async () => {
        ...
        assignmentFindMock.mockReturnValue(buildAssignmentQuery(assignments));
        userFindMock.mockReturnValue(BUILD_USER_QUERY([makeEmployee()]));
        assignments[0].courseId = makeCourse();
        await requireModule().sendCourseReminders();
        ...
        expect(assignmentUpdateMock).toHaveBeenCalledWith('a1', { lastReminderSentAt: expect.any(Date) });
    });
